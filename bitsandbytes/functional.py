...
def quantize_4bit(
    ...
    # ------------------------------------------------------------------
    #  ⚠  Fallback path when native quantization op is unavailable.
    # ------------------------------------------------------------------
    try:
        _op_namespace = torch.ops.bitsandbytes
        _ = getattr(_op_namespace, "quantize_4bit")  # will raise if missing
    except AttributeError:
        # Use the pure-Python TorchScript implementation registered in the
        # torchscript backend.  This guarantees that CPU/MPS testing passes on
        # developer machines that lack a compiled C++/CUDA extension.
        from bitsandbytes.backends.torchscript.ops import torchscript_quantize_4bit as _ts_quant

        _out, _absmax = _ts_quant(A, blocksize, quant_type, quant_storage)
    else:
